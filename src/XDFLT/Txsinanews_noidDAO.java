...
public class Txsinanews_noidDAO extends DaoFrame{
	private String tableNamenoid;
	private IDBConnector market;
	public Txsinanews_noidDAO(String tableNamenoid  , IDBConnector market){
		this.tableNamenoid = tableNamenoid;
	    this.market = market;
	}
	...
	public void modify(Object[] tempMap) throws Exception {
		...
		Txsinanews_noid[] listMap_noid = (Txsinanews_noid[])tempMap;
		...
		try{
			...
			logger.info(this.getClass().getName()+" 本次資料共"+listMap_noid.length+"筆");
			//建立表格
			...
			int[] ok = new int[listMap_noid.length] ;
			for(int j=0;j<listMap_noid.length;j++){
				Txsinanews_noid obj = listMap_noid[j];
				try{
			...
			for(int i = 0;i<listMap_noid.length;i++){
				//已 update 過的不再新增
				if(ok[i]==1)continue ;
				Txsinanews_noid obj = listMap_noid[i];
				try{
					pstmt.clearParameters();
	...
	public String getInsertSQL(){
		...
		sql.append(" INSERT INTO "+tableNamenoid);
	...
	public String getUpadteSQL(){
		...
		sql.append(" UPDATE "+tableNamenoid);
